...
# Model choice: gpt-4o-mini is cheap/fast. Swap to "gpt-4o" if you want
# higher-quality reasoning on messy, multi-document evidence chains —
# this spec asks the model to cross-reference claims across documents,
# which is exactly the kind of task that benefits from the bigger model
...
# idea into the miss path: 5 independent samples already give you
...
# blending 1-2 results and calling it an "ensemble."
ENSEMBLE_MIN_SUCCESSFUL = 3
...
def _build_system_prompt() -> str:
    ...
    dimension_examples = ",\n    ".join(
        f'{{"label": "{d.label}", "score": number, "description": string}}'
        for d in DIMENSIONS
    )
...
def _blend_ensemble_runs(run_results: list) -> dict:
    ...
    for label in labels:
        scores_by_run = []  # (run_index, score, description)
        for i, run in enumerate(run_results):
            for d in run.get("dimensions", []) or []:
                if d.get("label") == label:
                    try:
                        scores_by_run.append((i, float(d.get("score", 0)), d.get("description", "")))
                    except (TypeError, ValueError):
                        pass
                    break
        ...
        raw_scores = [s for _, s, _ in scores_by_run]
        blended_score = _round_to_half(_mode_with_mean_fallback(raw_scores))

        closest_run_idx, _, closest_desc = min(
...
def _run_one_scoring_attempt(client, base_messages: list, verified_achievement_count: int) -> dict:
    """
...
def analyze_documents(combined_text: str, extra_context: str = "") -> CVAnalysis:
    ...
    for _ in range(ENSEMBLE_RUNS):
        try:
            run_results.append(
                _run_one_scoring_attempt(client, base_messages, verified_achievement_count)
            )
