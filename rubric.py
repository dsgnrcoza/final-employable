...
def mechanical_ats_check(extracted_text: str, skill_labels: List[str] = None) -> dict:
    ...
    if skill_labels:
        ...
        if ratio >= 0.6:
            score += 1.0
            findings.append(f"{len(skills_in_body)}/{len(skill_labels)} listed skills appear as keywords in the document body")
        elif ratio >= 0.3:
            score += 0.5
            findings.append(f"Only {len(skills_in_body)}/{len(skill_labels)} listed skills appear in body text — more keyword integration would improve ATS ranking")
        else:
            findings.append(f"Very few listed skills appear in document body text ({len(skills_in_body)}/{len(skill_labels)}) — skills may only appear in a separate skills section, which some ATS systems weight lower")
