...
def api_onboarding_upload():
    ...
    for f in files:
        ...
        if not _allowed_file(f.filename):
            ...
            continue
        saved.append(pipeline.save_uploaded_file(user["id"], f))
...
def api_onboarding_check():
    user = auth.current_user()
    return jsonify(pipeline.check_identity_conflict(user["id"]))
...
def api_onboarding_remove_document(document_id):
    ...
    return jsonify({"ok": True, **pipeline.check_identity_conflict(user["id"])})
...
def api_roadmap_complete():
    """AI evaluates whether a selected document fulfills a roadmap objective. Awards points if it does."""
    from openai import OpenAI
    import extract as _extract
    user = auth.current_user()
    data = request.get_json(force=True)
    ...
    system = (
        ...
        '{"fulfilled": true/false, "reason": "one short sentence explaining your verdict", "points": 0.0, "steps": []}\n'
        "Rules:\n"
        "- points: 0.1–1.5 if fulfilled (based on quality), 0 if not fulfilled.\n"
