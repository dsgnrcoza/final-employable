    ...
    try:
        cur = conn.execute(
            """INSERT INTO users (username, password_hash, security_question,
               ...
               VALUES (?, ?, ?, ?, ?)""",
            (username, password_hash, security_question, security_answer_hash, now_iso()),
        )
        conn.commit()
...
def add_document(user_id, filename, stored_path, file_type, content=""):
    ...
    try:
        cur = conn.execute(
            """INSERT INTO documents (user_id, filename, stored_path, file_type, content, uploaded_at)
               VALUES (?, ?, ?, ?, ?, ?)""",
            (user_id, filename, stored_path, file_type, content or "", now_iso()),
        )
        conn.commit()
        return cur.lastrowid
    finally:
